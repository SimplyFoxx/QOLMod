-- =============================================================================
-- QOLMod: Robust Bag Management for WoW 3.3.5a
-- =============================================================================
-- Logic Path: [Normal/Gear] -> [Reagents] -> [Consumables] -> [Empty] -> [Whitelist]
-- =============================================================================
...
-- Hearthstone (6948) is Index 1, so it moves to the ABSOLUTE LAST slot.
local whitelist = { 
    6948,   -- Hearthstone
    9017,   -- Book of Maf
...
-- QUEUE SYSTEM
-- Processes actions with 0.1s delay to satisfy server anti-spam.
QOLMod:SetScript("OnUpdate", function(self, elapsed)
    if #q == 0 then 
        if startTime > 0 then
            local dur = floor((GetTime() - startTime) * 10) / 10
            print("|cFF00FF00[QOLMod]|r Sort finished in " .. dur .. "s.")
            startTime = 0
        end
        return 
    end
...
local function SortBags()
    ...
    local allSlots = {}      -- {b, s}
    local linearInventory = {} -- Current items with Unique IDs (Original slot index)

    -- 1. Scan everything into linear lists
    for bag = 0, 4 do
        ...
        for slot = 1, numSlots do
            ...
            if link then
                ...
                table.insert(linearInventory, {
                    uid = slotIdx, -- Unique Instance ID
                    id = id, n = name, q = qual, t = iType, st = iSub,
    ...
    -- 2. Sort Logic: Categorize contents
    local normalPile = {}
    local reagentPile = {}
    ...
    -- 4. Execution Loop: Linear Correction (Slot 1 to N)
    local currentMap = {}
    ...
    for i = 1, #allSlots do
        ...
        if currentMap[i] ~= goalUID then
            -- Find where goalUID is currently located
            for j = i + 1, #allSlots do
                if currentMap[j] == goalUID then
                    ...
                    local t = allSlots[i]
                    AddTask(function() 
                        PickupContainerItem(s.b, s.s) 
                        PickupContainerItem(t.b, t.s) 
                    end)
                    
                    -- Swap Virtually (Memory)
...
local function CreateUI()
    ...
    local function CreateDrop(label, y, var, options)
        ...
        dd:SetPoint("TOPLEFT", 10, y - 15)
        UIDropDownMenu_SetWidth(dd, 160)
        
        UIDropDownMenu_Initialize(dd, function()
            for _, o in ipairs(options) do
                local info = UIDropDownMenu_CreateInfo()
                info.text = (type(o) == "table") and o.t or o
                info.value = (type(o) == "table") and o.v or o
    ...
    local qOpts = {
        {t = "Poor (Grey)", v = 0}, {t = "Common (White)", v = 1}, 
        {t = "Uncommon (Green)", v = 2}, {t = "Rare (Blue)", v = 3}, {t = "Epic (Purple)", v = 4}
    }
    CreateDrop("Sell Threshold (and under):", -50, "sellMaxQual", qOpts)
    ...
    cb:SetPoint("TOPLEFT", 20, -170)
    _G[cb:GetName() .. "Text"]:SetText("Destroy Quest/0-Value Items")
    cb:SetChecked(QOLModDB.destroyUnsellables)
    cb:SetScript("OnClick", function(self) 
        QOLModDB.destroyUnsellables = self:GetChecked() 
    end)
...
SlashCmdList["QOL"] = function(msg)
    if msg == "sort" then 
        ...
        if QOLMenu:IsShown() then QOLMenu:Hide() else QOLMenu:Show() end
    end
end